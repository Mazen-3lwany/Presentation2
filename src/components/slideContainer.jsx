export default function SlideContainer({ slides, currentSlide, children }) {
  return (
    <div className="relative w-full max-w-6xl h-[85vh] bg-linear-to-br from-gray-900/90 to-gray-800/90 backdrop-blur-2xl border border-emerald-400/30 rounded-3xl shadow-2xl overflow-hidden group">
      ...
      <div className="absolute inset-0 bg-linear-to-br from-emerald-400/0 via-emerald-400/20 to-cyan-400/0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none rounded-3xl"></div>
      ...
      <div className="relative w-full h-full p-16">
        {slides.map((slide, index) => (
          <div
            key={index}
            className={`absolute inset-0 p-16 transition-all duration-700 ease-out ${
              index === currentSlide
